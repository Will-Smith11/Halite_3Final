...
class Ship(Entity):
    ...
    def __init__(self, owner, id, position, halite_amount, moving_around):
        super().__init__(owner, id, position)
        self.halite_amount = halite_amount
        self.moving_around = False
    ...
    @property
    def route(self):
        return self.moving_around
    
    @property
    def take_second(self):
        self.moving_around = True
    ...
    def _generate(player_id):
        ...
        if ship_id in Ship.__ships.keys():    
            ...
            new_ship = Ship(player_id, ship_id, Position(x_position, y_position), halite, moving_around)
            Ship.__ships[ship_id] = new_ship
            return ship_id, new_ship

    def __repr__(self):
        return "{}(id={}, {}, cargo={} halite, moving_around={})".format(self.__class__.__name__,
                                                       self.id,
                                                       self.position,
                                                       self.halite_amount,
                                                       self.moving_around)

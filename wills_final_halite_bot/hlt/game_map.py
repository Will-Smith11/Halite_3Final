...
from .positionals import Direction, Position, get_direction
...
class MapCell:
    ...
    def enemy_ship(self):
        ...
        return self.e_ship is not None

    @property
    def insper_cell(self):

        return self.insper is not None


    def mark_e_ship(self, ship):
...
class GameMap:
    ...
    def better_nav(self,ship,des):
        if ship.position != des:
            if not ship.moving_around:
                try:
                    direction = self.get_unsafe_moves(ship.position,des)[0]
                    target = ship.position.directional_offset(direction)
                    if not self[target].is_occupied:
                        if not self[target].enemy_ship:
                            self[target].mark_unsafe(ship)
                            self[ship.position].ship = None
                            return direction
                        else:
                            if self[target].has_structure:
                                self[target].mark_unsafe(ship)
                                self[ship.position].ship = None
                                return direction
                    ...
                    for direction in self.get_unsafe_moves(ship.position, des):
                        target = ship.position.directional_offset(direction)
                        if not self[target].is_occupied:
                            if not self[target].enemy_ship:
                                self[target].mark_unsafe(ship)
                        ...
                        return Direction.Still  
            try:
                direction = self.get_unsafe_moves(ship.position,des)[1]
                target = ship.position.directional_offset(direction)
                if not self[target].is_occupied:
                    if not self[target].enemy_ship:
                        self[target].mark_unsafe(ship)
    ...
    def get_direction(self,ship,des):
        if ship.position != des:
            if not ship.moving_around:
                try:
                    direction = self.get_unsafe_moves(ship.position,des)[0]
    ...
    def path(self,ship,des):
        if des != ship:
            for direction in self.get_unsafe_moves(ship,des):  
                return direction
        else:
            return Direction.Still       
    
    def switch(self,ship, des):
        for direction in self.get_unsafe_moves(ship.position,des):
            target = ship.position.directional_offset(direction)        
            self[target].mark_unsafe(ship)
            return direction
    ...
    def _update(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                self[Position(x, y)].e_ship = None
                self[Position(x, y)].insper = None
